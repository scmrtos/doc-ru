...
Для определения типов процессов используется стандартное средство С+⁠+ – шаблоны (templates), что позволило получить "компактные" типы процессов, в которых содержатся все необходимые внутренности, включая и непосредственно стек процесса, который у всех процессов имеет разный размер и задаётся индивидуально.
...
["Листинг 1. Исполняемая функция процесса"](overview.md#process-exec) иллюстрирует пример типовой процессной функции.
